...
"""
🌤️ WORKFLOW PREFECT - Sincronização Meteorológicos BigQuery

Este workflow usa Prefect para orquestrar a sincronização de dados METEOROLÓGICOS
...
@task(name="Verificar Lacunas de Dados Meteorológicos", log_prints=True)
def verificar_lacunas_dados_meteorologicos() -> dict:
    """Verifica se há dados meteorológicos no NIMBUS antes da data atual que ainda não foram coletados.
    ...
    try:
        ...
        # Verificar se há dados meteorológicos no NIMBUS entre última sincronização e data atual
        # Usar a mesma estrutura da query de sincronização (GROUP BY)
...
def sincronizacao_meteorologicos_flow() -> dict:
    """Flow principal para sincronização incremental de dados meteorológicos."""
    print("=" * 80)
    print("🌤️ SINCRONIZAÇÃO INCREMENTAL METEOROLÓGICOS - BigQuery")
    print("=" * 80)
    print(f"⏰ Início: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📊 Tabela: meteorologicos")
    print()
    ...
    resultado_lacunas = verificar_lacunas_dados_meteorologicos()
    ...
    resultado_sync = sincronizar_meteorologicos_incremental()
    ...
    status_bq = verificar_status_bigquery_meteorologicos()
...
if __name__ == "__main__":
    ...
    - python prefect_workflow_meteorologicos.py --run-once
    
    OPÇÃO 2: Prefect Cloud
    - prefect deploy prefect_workflow_meteorologicos.py:sincronizacao_meteorologicos_flow --pool seu-work-pool
    ...
    if '--run-once' in sys.argv:
        print("🔄 Executando sincronização meteorológicos (sem criar deployment)...")
        resultado = sincronizacao_meteorologicos_flow()
        sys.exit(0 if resultado.get('sucesso', False) else 1)
    else:
        try:
            # Criar deployment (requer Prefect Cloud ou Local rodando)
            sincronizacao_meteorologicos_flow.serve(
                name="sincronizacao-bigquery-meteorologicos",
                cron="*/5 * * * *",  # A cada 5 minutos
                description="Sincronização incremental de dados meteorológicos do NIMBUS para BigQuery (tabela: meteorologicos)"
            )
            ...
            if e.response.status_code == 401:
                ...
                print("   2. Executar sem deployment:  python prefect_workflow_meteorologicos.py --run-once")
