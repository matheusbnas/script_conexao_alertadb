...
✅ APÓS executar exportar_pluviometricos_servidor166_bigquery.py (carga inicial)
...
⚠️ NÃO USE se:
   ❌ A tabela BigQuery estiver vazia (use exportar_pluviometricos_servidor166_bigquery.py primeiro)
   ❌ Você quer carregar dados históricos (use exportar_pluviometricos_servidor166_bigquery.py)

═══════════════════════════════════════════════════════════════════════════
🚀 COMO USAR:
═══════════════════════════════════════════════════════════════════════════

1. PRIMEIRO: Execute exportar_pluviometricos_servidor166_bigquery.py para carga inicial
2. Configure o arquivo .env com as credenciais
3. Execute: python sincronizar_pluviometricos_servidor166_bigquery.py --once
...
def sincronizar_incremental():
    ...
    if ultima_sincronizacao == datetime(1997, 1, 1, tzinfo=timezone.utc):
        ...
        print("   Execute PRIMEIRO: python scripts/bigquery/exportar_pluviometricos_servidor166_bigquery.py")
...
def main():
    ...
    try:
        if '--once' in sys.argv:
            ...
            print("   python scripts/bigquery/sincronizar_pluviometricos_servidor166_bigquery.py --once")
